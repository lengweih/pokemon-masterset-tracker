...
// Single source of truth for the master set: every Prismatic Evolutions card
// (#001-#180) and the rules that determine each card's master (booster-pack)
// variants. No products are tied to master-set cards.
...
export type MasterSetCardData = readonly [
  number: string,
  name: string,
...
export const masterSetCardData = [
...
export const getMasterSetVariants = (
  rarity: CollectionCardRarity,
  type: CollectionCardType,
) => {
...
// Master-set card display name by number. Lets other data (e.g. grandmaster
// promo product lists) reference a card by number without duplicating its name.
export const masterCardNameByNumber: Record<string, string> = Object.fromEntries(
  masterSetCardData.map(([number, name]) => [number, name]),
);
